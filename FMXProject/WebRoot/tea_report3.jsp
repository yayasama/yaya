...
<!--教师审核学生提交的结题报告，显示学生提交的结题报告-->
<html>
<script>
function firstpage() {
	var strBack = "listjieti.review";
...
function before(currentPage, pageSize) {
	...
	var strBefore = "page_report3.review?currentPage=" + this.currentPage
...
function back(currentPage, pageSize, pageCount) {
	this.currentPage = currentPage + 1;
	this.pageSize = pageSize;
	var strBack = "page_report3.review?currentPage=" + this.currentPage
			+ "&pageSize=" + this.pageSize;
...
<link type="text/css" rel="stylesheet" href="css/style.css" />
  <div id="m_right">
	...
	<!-- 接受传入的对象 -->
	<c:forEach items="${requestScope.rep3}" varStatus="status" var="r3" >
	...
	<tr class="stu_table2">	
			...
			<td>${r3.result}</td>
			<td>
			<c:if test="${r3.result == '待审核'}">
			<a href="<c:url value="report3.review?id=${r3.id}" /> ">审核</a>		
			</c:if>
			</td>
			</tr></c:forEach>		
	</table>
