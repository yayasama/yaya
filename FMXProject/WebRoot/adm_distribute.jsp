...
function before(currentPage, pageSize) {
	...
	var strBefore = "page_dis.admin?currentPage=" + this.currentPage
...
function back(currentPage, pageSize, pageCount) {
	this.currentPage = currentPage + 1;
	this.pageSize = pageSize;
	var strBack = "page_dis.admin?currentPage=" + this.currentPage
			+ "&pageSize=" + this.pageSize;
...
<link type="text/css" rel="stylesheet" href="css/style.css" />
 <div id="m_right"> 
  <h3 style="position:absolute;top:100px;left:300px;">已经审核通过的项目</h3><br><br>
	 <table class="table-1"> 
		<tr class="stu_table1">
	...
	<tr class="stu_table2">	
			...
			<td>${p.prostate}</td>
			<td>	
			<form action="prostate.admin?id=${p.id}" method="post">
			<c:if test="${p.prostate !='已发布'}">	
			  <button name="prostate" value="已发布" style="color: blue;" onclick="prostate.admin?id=${p.id}" >发布</button>
			</c:if>
			</form>
			</td>
			</tr></c:forEach>		
